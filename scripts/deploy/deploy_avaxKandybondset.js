...
async function main() {
    ...
    const avaxStripBondAddress = "0x46207878E76D903f830980A42Ec16716ae655369";
    ...
    const avaxStripLPTokenAddress = "0xD20d23772324737A6FD376002DC8E0842bA4551a";
    ...
    console.log("start");
    const AvaxStripBond = await ethers.getContractFactory('StripBondDepository');
    const avaxStripBond = await AvaxStripBond.attach(avaxStripBondAddress);
    console.log("start set");
    // Set AVAX bond terms
    await avaxStripBond.initializeBondTerms(BondBCV, minBondPrice, maxBondPayout, bondFee, maxBondDebt, intialBondDebt, bondVestingLength);
    console.log("Set AvaxStrip bond terms");

    // Set staking for AVAX bond
    await avaxStripBond.setStaking(stakingAddress, 0);
    await avaxStripBond.setStaking(stakingHelperAddress, 1);
    ...
    await treasury.queue('5', avaxStripLPTokenAddress);
    await treasury.toggle('5', avaxStripLPTokenAddress, bondCalcAddress);
    console.log("queue and toggle LP token reserve token");

    // queue and toggle WAVAX bond reward manager
    await treasury.queue('4', avaxStripBond.address);
    await treasury.toggle('4', avaxStripBond.address, zeroAddress);
