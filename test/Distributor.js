...
describe("Distributor Test", function() {
  ...
  describe("Distributor", function() {
    it("Strip", async () => {
      console.log("\tcheck strip address of Distributor");
      expect(await distributor.Strip()).to.equal(strip.address);
    });

    it("epochLength", async () => {
      console.log("\tcheck epochLength of Distributor");
      expect(await distributor.epochLength()).to.equal(1920);
    });

    it("treasury", async () => {
      console.log("\tcheck treasury address of Distributor");
      expect(await distributor.treasury()).to.equal(treasury.address);
    });

    it("addRecipient()", async() => {
      console.log("\tcheck addRecipient() function")
      distributor.addRecipient(addr1.address, initialRewardRate);
      let real = await distributor.info("1");
      expect(real.rate.toString()).to.equal(initialRewardRate);
      expect(real.recipient).to.equal(addr1.address);
    });
