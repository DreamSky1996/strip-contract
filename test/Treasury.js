...
describe("Treasury Test", function() {
  ...
  describe("Treasury", function() {
    it("Strip", async () => {
      console.log("\tcheck strip address of treasury");
      expect(await treasury.Strip()).to.equal(strip.address);
    });

    it("isReserveToken", async () => {
      console.log("\tcheck reserve token");
      expect(await treasury.isReserveToken(strip.address)).to.equal(false);
      expect(await treasury.isReserveToken(mim.address)).to.equal(true);
    });

    it ("reward manager", async () => {
      console.log("\tcheck reward manager");
      expect(await treasury.isRewardManager(distributor.address)).to.equal(true);
    });
    it("MIM bond reserve depositor", async () => {
      console.log("\tcheck MIM bond reserve depositor");
      expect(await treasury.isReserveDepositor(mimBond.address)).to.equal(true);
    });

    it("reserve depositor", async () => {
      console.log("\tcheck reserve depositor");
      expect(await treasury.isReserveDepositor(deployer.address)).to.equal(true);
    });

    it("liquidity depositor", async () => {
      console.log("\tcheck liquidity depositor");
      expect(await treasury.isLiquidityDepositor(deployer.address)).to.equal(true);
    });
    it("deposit()", async () => {
