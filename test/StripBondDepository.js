...
describe("StripBondDepository Test", function() {
  ...
  describe("StripBondDepository", function() {
    beforeEach(async () => {
      await treasury.deposit('9000000000000000000000000', mim.address, '8400000000000000');
    });
    it("staking", async () => {
      console.log("\tcheck staking address of StripBondDepository");
      expect(await mimBond.staking()).to.equal(staking.address);
    });

    it("stakingHelper", async () => {
      console.log("\tcheck stakingHelper address of StripBondDepository");
      expect(await mimBond.stakingHelper()).to.equal(stakingHelper.address);
    });

    it("terms", async() => {
      console.log("\tcheck initTerms funtion of StripBondDepository");
      let terms = await mimBond.terms();
      expect(terms.controlVariable.toString()).to.equal(mimBondBCV);
      expect(terms.minimumPrice.toString()).to.equal(minBondPrice);
      expect(terms.maxPayout.toString()).to.equal(maxBondPayout);
      expect(terms.fee.toString()).to.equal(bondFee);
      expect(terms.maxDebt.toString()).to.equal(maxBondDebt);
      expect(terms.vestingTerm).to.equal(28800);
    });
