...
describe("Staking Test", function() {
  ...
  describe("StripStaking", function() {
    it("Strip", async () => {
      console.log("\tcheck strip address of StripStaking");
      expect(await staking.Strip()).to.equal(strip.address);
    });

    it("SStrip", async () => {
      console.log("\tcheck sStrip address of StripStaking");
      expect(await staking.Memories()).to.equal(sStrip.address);
    });

    it("setContract:distributor", async () => {
      console.log("\tcheck setContract('0', distributor.address)");
      expect(await staking.distributor()).to.equal(distributor.address);
    });
    
    it("setContract:stakingWarmup", async () => {
      console.log("\tcheck setContract('1', stakingWarmup.address)");
      expect(await staking.warmupContract()).to.equal(stakingWarmup.address);
    });
