...
describe("SStripERC20 Test", function() {
  ...
  describe("SStripERC20", function() {
    it('decimals', async () => {
      console.log("\tcheck decimals");
      expect(await sStrip.decimals()).to.equal(9);
    });

    it('name&symbol', async () => {
      console.log("\tcheck name and symbol");
      expect(await sStrip.name()).to.equal("SStrip");
      expect(await sStrip.symbol()).to.equal("SSTRIP");
    });

    it('index', async () => {
      console.log("\tcheck index");
      expect(await sStrip.index()).to.equal(initialIndex);
    });

    it("stakingContract", async() => {
      console.log("\tcheck stakingContract address");
      expect(await sStrip.stakingContract()).to.equal(staking.address);
    });
  });
