...
describe("StripERC20 Test", function() {
  ...
  describe("StripERC20", function() {

    it('owner', async () => {
      console.log("\tcheck owner");
      expect(await strip.owner()).to.equal(deployer.address);
    });

    it('decimals', async () => {
      console.log("\tcheck decimals of Strip");
      expect(await strip.decimals()).to.equal(9);
    });

    it('name&symbol', async () => {
      console.log("\tcheck name and symbol of Strip");
      expect(await strip.name()).to.equal("Strip");
      expect(await strip.symbol()).to.equal("STRIP");
    });

    it('setVault()', async () => {
      console.log("\tcheck setVault function of strip");
      expect(await strip.vault()).to.equal(treasury.address);
    });

    it('mint(account_, amount_)', async () => {
      console.log("\tcheck mint funtion");
      await strip.setVault(addr1.address);
      await strip.connect(addr1).mint(addr1.address, "1000000");
      expect(await strip.balanceOf(addr1.address)).to.equal("1000000");
    });

    it('approve(spender, amount)', async () => {
      console.log("\tcheck approve function");
      await strip.connect(addr1).approve(addr2.address, largeApproval);
      expect(await strip.allowance(addr1.address, addr2.address)).to.equal(largeApproval);
    });
  });
